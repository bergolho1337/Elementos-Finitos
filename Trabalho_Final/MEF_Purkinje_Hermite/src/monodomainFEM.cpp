...
MonodomainFEM* newMonodomainFEM (int argc, char *argv[])
{
    ...
    fscanf(file,"%d %d %lf",&monoFEM->nElem,&monoFEM->nPoints,&monoFEM->dx);
    // Lendo os pontos
    monoFEM->points = (Point*)malloc(sizeof(Point)*monoFEM->nPoints);
    for (int i = 0; i < monoFEM->nPoints; i++)
      fscanf(file,"%lf %lf %lf",&monoFEM->points[i].x,&monoFEM->points[i].y,&monoFEM->points[i].z);
    // Lendo os elementos
...
void kirchoffCondition_Matrix (MonodomainFEM *monoFEM)
{
  ...
  for (int i = 0; i < monoFEM->bif.size(); i++)
  {
    ...
    for (int j = 0; j < monoFEM->bif[i].links.size(); j++)
    {
